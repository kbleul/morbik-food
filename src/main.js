    ...
    <div className="h-screen overflow-y-scroll">
      <article className="lg:ml-2 mt-16 ">
        <section className="catagories_section">
          {
            catagories.map(item => (
              <button className="catagories_btn" key={item.strCategory} onClick={() => { set_choicetype("c"); setChoice(item.strCategory); set_togglerecipe_page(false); }}>{item.strCategory}</button>
            ))
          }
        </section>
        ...
        {togglerecipe_page ? <RecipePage foodid={recipefor} similarfoods={frontfood_list} /> :
          <div>
            ...
            {frontfood_list.length === 0 ? <div className="main_foodlist-container">
               ...
               <p>No available food for this ingrident.</p></div> :
                frontfood_list.map(item => (
                  <div className="hover:brightness-90 px-1" key={item.idMeal} onClick={() => { set_recipefor(item.idMeal); set_togglerecipe_page(true); }}>
                    ...
                    <p className="item_name-p" key={item.strMeal}>{item.strMeal}</p>
                  </div>
                ))
              }
            </section>
