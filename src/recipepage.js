...
const RecipePage = ({ foodid, similarfoods }) => {
  ...
  return (<section className="mt-8">
      ...
      <div className="mt-16">
        {
          recipedata.map(item => (
            <div key={`${item.strMeal} ${item.strMeal} ${item.idMeal}`}>
              <section key={`${item.strMeal} ${item.strMeal} ${item.idMeal}1`} className="flex flex-col md:flex-row items-center ">
                <div key={`${item.strMeal} ${item.strMeal} ${item.idMeal}2`} className="mb-20 md:mb-0 md:ml-2 flex flex-col w-11/12 justify-center md:w-2/4">
                  <div className="flex justify-center"><img className="ml-1 w-full md:w-[440] " key={item.strMealThumb} src={item.strMealThumb} alt={item.strMeal} /></div>
                  <div key={`${item.strMeal} ${item.strMeal} ${item.idMeal}3`} className="flex flex-col md:flex-row justify-between text-base">
                    <h4 className="mt-4 md:mt-0 text-center border-b-2  font-black pl-2 md:w-3/5" key={item.strMeal}>{item.strMeal} </h4>
                    {isfavorite ?

                      <button key={`${item.idMeal}${item.idMeal}1`} className="order-first md:order-last pr-4 text-red-400 ml-8 self-end hover:border-b-2 hover:border-red-300 flex" onClick={() => {set_isfavorite(false); removeFavorite("food", item.idMeal)}}><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1.5em" height="1.5em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><path fill="currentColor" d="M11 15h2V9h3l-4-5l-4 5h3z" /><path fill="currentColor" d="M20 18H4v-7H2v7c0 1.103.897 2 2 2h16c1.103 0 2-.897 2-2v-7h-2v7z" /></svg><p className="md:ml-2">Saved</p></button> :

                      <button key={`${item.idMeal}${item.idMeal}2`} className="order-first md:order-last  text-red-400 border-b-2 border-red-300 hover:opacity-70 ml-8 self-end flex " onClick={() => { set_isfavorite(true); saveFavorites("food", item.idMeal) }}><svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" width="1.5em" height="1.5em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><path fill="currentColor" d="M19 12v7H5v-7H3v7c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2v-7h-2zm-6 .67l2.59-2.58L17 11.5l-5 5l-5-5l1.41-1.41L11 12.67V3h2v9.67z" /></svg><p className="md:ml-2">Add to Favories</p></button>
                    }
                  </div>
                </div>

                {!showvideo && <button key={`${item.strMeal} ${item.strMeal} ${item.idMeal}5`} className="border-1 rounded-full bg-amber-300 px-8 py-2 flex flex-row justify-between font-mono md:ml-24 hover:bg-amber-400 dark:bg-amber-400 dark:hover:opacity-70" onClick={() => set_showvideo(true)}>
                ...
                {
                  (showvideo && youtubeid !== "") && <div className="md:ml-8 w-full md:w-1/2" key={`${item.strMeal} ${item.strMeal} ${item.idMeal}6`}>
                    <iframe className="w-full md:w-[440]" key={`${item.strMeal} ${item.strMeal} ${item.idMeal}9`}  height="425" src={`https://www.youtube.com/embed/${youtubeid}`} title="YouTube video player" frameborder="0" allowFullScreen></iframe>
              ...
              <section className="flex flex-col justify-items-center items-center">
                {ingredientamount.length > 0 && <div className="bg-gray-100 rounded-r-lg w-full md:w-5/12 my-20 py-8 pl-4 font-serif dark:bg-gray-700">
                ...
                <div className="w-full md:w-3/4 border-l-2 border-l-green-700 px-2 md:pl-8 my-40 font-serif leading-6  md:leading-8">
    ...
    {recipedata.length !== 0 && <h2 className="capitalize text-xl leading-10 font-light pl-2 text-center  md:text-left md:ml-24">Similar Foods</h2>}

    <div className="h-96 overflow-y-scroll md:ml-24 ">

      {similarfoods.map(item => (<div className="float-left w-1/3 md:w-1/5 md:ml-2" onClick={() => set_currentfood_id(item.idMeal)} key={`${item.idMeal}${item.strMealThumb}${item.strMeal}`}>
        {
