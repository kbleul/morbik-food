          ...
          recipedata.map(item => (
            <div key={`${item.strMeal} ${item.strMeal} ${item.idMeal}`}>
              <section key={`${item.strMeal} ${item.strMeal} ${item.idMeal}1`} className="flex flex-row items-center">
                ...
                {!showvideo && <button key={`${item.strMeal} ${item.strMeal} ${item.idMeal}5`} className="border-1 rounded-full bg-amber-300 px-8 py-2 flex flex-row justify-between font-mono ml-24 hover:bg-amber-400 dark:bg-amber-400 dark:hover:opacity-70" onClick={() => set_showvideo(true)}>
