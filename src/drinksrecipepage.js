...
const RecipePage = ({ drinkid, similardrinks }) => {
  ...
  const getRecipe = async () => {
    ...
    try {
      const result = await axios(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${currentdrink_id}`)
  ...
  return (<section className="mt-8">
    ...
    <div className="h-96 overflow-y-scroll ml-24 ">
      ...
      {similardrinks.map(item => (<div className="float-left w-1/5 ml-2" onClick={() => set_currentdrink_id(item.idMeal)} key={`${item.idMeal}${item.strMealThumb}${item.strMeal}`}>
        {
