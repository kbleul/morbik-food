...
const RecipePage = ({ drinkid, similardrinks }) => {
  ...
  return (<section className="mt-8">
    ...
    <img src={loading} alt="loading" className="w-12 h-12" /> 
      ...
      <div className="mt-16">
        {
          recipedata.map(item => (
            <div key={`${item.strDrink} ${item.strDrink} ${item.idDrink}`}>
              <section key={`${item.strDrink}1`} className="flex flex-col md:flex-row items-center">
                <div  key={`${item.strDrink}2`}  className="mb-20 md:mb-0 md:ml-2 flex flex-col w-11/12 justify-center md:w-2/4" >
                  <div  key={`${item.strDrink}3`} className="flex justify-center"><img className="ml-1 w-full md:w-[440] " key={item.strDrinkThumb} src={item.strDrinkThumb} alt={item.strDrink} width="480" height="315" /></div>

                   <div  key={`${item.strDrink}4`} className="flex flex-col md:flex-row justify-between text-base">
                    <h4 className="mt-4 md:mt-0 text-center border-b-2  font-black pl-2 md:w-3/5" key={item.strDrink}>{item.strDrink} </h4>
              ...
              <section  key={`${item.strDrink}5`} className="flex flex-col justify-items-center items-center">
                {ingredientamount.length > 0 && <div  key={`${item.strDrink}6`} className="bg-gray-100 rounded-r-lg w-full md:w-5/12 my-20 py-8 pl-4 font-serif dark:bg-gray-700 ">
                ...
                <div  key={`${item.strDrink}8`} className="w-full md:w-3/4 border-l-2 border-l-green-700 px-2 md:pl-8 my-40 font-serif leading-6  md:leading-8">
     ...
     {recipedata.length !== 0  && <h2 className="capitalize text-xl leading-10 font-light pl-2 text-center  md:text-left md:ml-24">Similar Drinks</h2>}

    <div className="h-96 overflow-y-scroll md:ml-24 ">

      {similardrinks.map(item => (<div className="float-left w-1/3 md:w-1/5 md:ml-2" onClick={() => set_currentdrink_id(item.idDrink)} key={`${item.idDrink}${item.strDrinkThumb}${item.strDrink}`}>
        {
