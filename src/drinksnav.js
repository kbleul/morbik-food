...
const Navigation = ({ setnavChoice, set_choicetype, set_togglerecipe_page }) => {
  ...
  return (
    <section className="top_section-drink">
      <div>
        <div className="nav_title-container">
          ...
          <button className="nav_title-button" onClick={() => showChoices("alcholic")}>{showalcholic ? "↑" : "↓"}</button>
        </div>
        {!showalcholic_choiceresults ? <div className="loading_container"> 
        <img className="w-10 h-10" src={loading} alt="loading" /></div> :
          <div className={showalcholic ? "flex flex-col items-center" : "hidden"}>
            {alcholic_choiceresults.map(item => (
              <button className={picked === item.strAlcoholic ? "nav_area-options-btn--clicked" :"nav_area-options-btn"} onClick={() => { set_togglerecipe_page(false); appendUnderscore(item.strAlcoholic); set_choicetype("a"); set_picked(item.strAlcoholic); toogleside_menu();  }}
        ...
        <div className="nav_title-container">
          ...
          <button className="nav_title-button" onClick={() => showChoices("glass")}>{showglasses ? "↑" : "↓"}</button>
        </div>
        {!showglasses_choiceresults ? <div className="loading_container"> 
        <img  className="w-10 h-10" src={loading} alt="loading" /></div> :
          <div className={showglasses ? "flex flex-col items-center" : "hidden"}>
            {glasses_choiceresults.map(item => (
              <button className={picked === item.strGlass ? "nav_ingredient-option-btn--clicked" :"nav_ingredient-option-btn"} onClick={() => { set_togglerecipe_page(false); appendUnderscore(item.strGlass); set_choicetype("g"); set_picked(item.strGlass); toogleside_menu(); }}
        ...
        {!showingredient_choiceresults ? <div className="loading_container">  <img  className="w-10 h-10" src={loading} alt="loading" /></div> :
          <div className={showingredients ? "grid grid-cols-2" : "hidden"}>
            {ingredient_choiceresults.map(item => (
              <button className={picked === item.strIngredient1 ? "nav_ingredient-option-btn--clicked" :"nav_ingredient-option-btn"} onClick={() => { set_togglerecipe_page(false); appendUnderscore(item.strIngredient1); set_choicetype("i"); set_picked(item.strIngredient1); toogleside_menu(); }}
